...
SECRET_PATTERNS = [
    ...
    # Cloud credentials
    (r'aws[_-]?secret[_-]?access[_-]?key[_-]?[:=]\s*["\']?[A-Za-z0-9/+=]{40}["\']?', 'AWS Secret Key'),
    (r'aws[_-]?access[_-]?key[_-]?id[_-]?[:=]\s*["\']?[A-Z0-9]{20}["\']?', 'AWS Access Key'),
...
EXCLUDED_FILES = [
    ...
    '*.json', '*.yaml', '*.yml',  # Config files often have example values
]
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        for pattern, secret_type in SECRET_PATTERNS:
            ...
            for match in matches:
                ...
                issues.append((line_num, secret_type, match_text[:50] + '...' if len(match_text) > 50 else match_text))
