...
class PreCommitValidator:
    ...
    def validate_environment(self) -> ValidationResult:
        ...
        return ValidationResult(
            ...
            duration=duration,
            output="\n".join(output_lines),
            error=f"Missing tools: {', '.join(missing_tools)}" if missing_tools else None
        )
    ...
    def validate_code_quality(self) -> List[ValidationResult]:
        ...
        success, stdout, stderr = self.run_command([
            ...
            "healthcare-ai-app/src", "healthcare-ai-app/scripts", "healthcare-ai-app/tests",
            "mlops-platform/services", "mlops-platform/scripts"
        ])
        duration = time.time() - start_time
        ...
        results.append(ValidationResult(
            ...
            passed=success,
            duration=duration,
            output="No linting issues found" if success else f"Linting issues:\n{stdout}",
            error=stderr if stderr else None
        ))
        ...
        success, stdout, stderr = self.run_command([
            ...
            "healthcare-ai-app/src", "mlops-platform/services"
        ])
        duration = time.time() - start_time
        ...
        results.append(ValidationResult(
            ...
            passed=success,
            duration=duration,
            output="No type issues found" if success else f"Type checking issues:\n{stdout}",
            error=stderr if stderr else None
        ))
    ...
    def validate_security(self) -> List[ValidationResult]:
        ...
        success, stdout, stderr = self.run_command([
            ...
            "healthcare-ai-app/src", "healthcare-ai-app/scripts",
            "mlops-platform/services", "mlops-platform/scripts"
        ])
        duration = time.time() - start_time
    ...
    def validate_healthcare_specific(self) -> List[ValidationResult]:
        ...
        if data_script.exists():
            success, stdout, stderr = self.run_command([
                "python3", str(data_script)
            ], cwd=self.healthcare_dir)
        else:
            success, stdout, stderr = False, "", "Training data validation script not found"
    ...
    def validate_git_status(self) -> ValidationResult:
        ...
        staged_success, staged_files, _ = self.run_command(["git", "diff", "--cached", "--name-only"])
        if staged_success and staged_files:
            for file_path in staged_files.strip().split('\n'):
                if file_path:
                    # Check file content for sensitive data
                    file_success, file_content, _ = self.run_command(["git", "show", f":{file_path}"])
                    if file_success:
                        file_lower = file_content.lower()
                        for pattern in sensitive_patterns:
                            if pattern in file_lower and "example" not in file_lower:
                                issues.append(f"Potential sensitive data in {file_path}: contains '{pattern}'")
        ...
        return ValidationResult(
            ...
            passed=len(issues) == 0,
            duration=duration,
            output=f"Git status clean" if len(issues) == 0 else f"Issues found:\n" + "\n".join(issues),
            error=None
        )
